...
all_windows = set(file_one_scores.keys()).union(file_two_scores.keys())
for region in all_windows:
    file_one_region_mean_score = 0;
    if region in file_one_scores:
        file_one_region_mean_score = float(sum(file_one_scores[region])) / len(file_one_scores[region])
    file_two_region_mean_score = 0;
    if region in file_two_scores:
...
OutFileName = sys.argv[1] + '_DDMRs_above_' + sys.argv[3] + '%_' + '.gff'
...
for DDMR in DDMRs:
    OutFile.write('%s\t%s\t%s\t%s\t%s\t%s\t.\t.\n' % ( chromosome,'.', context, int(DDMR*1000+1), int((DDMR+1)*1000), DDMRs[DDMR]))
